...
import { maxItems, minItems, type } from "vue-vuelidate-jsonschema";
...
export const after = (after, fieldName) =>
  helpers.withParams({ after }, function (date, data) {
    // Get check date
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
      level
    );
...
export const after_or_equal = (after_or_equal, fieldName) =>
  helpers.withParams({ after_or_equal }, function (date, data) {
    // Get check date
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
    ...
    const checkDate = moment(
      get(dataWithParent, after_or_equal, after_or_equal)
    );
    if (!checkDate.isValid()) {
      return false;
    }
...
export const before = (before, fieldName) =>
  helpers.withParams({ before }, function (date, data) {
    // Get check date
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
...
export const before_or_equal = (before_or_equal, fieldName) =>
  helpers.withParams({ before_or_equal }, function (date, data) {
    // Get check date
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
    ...
    dataWithParent.today = moment().format("YYYY-MM-DD");
    const checkDate = moment(
      get(dataWithParent, before_or_equal, before_or_equal)
    );
    if (!checkDate.isValid()) {
      return false;
    }
...
export const regex = (expression) =>
  helpers.withParams({ expression }, (value) => {
...
export const requiredIf = (variable, expected, fieldName) =>
  helpers.withParams({ variable, expected }, function (value, data) {
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
      level
...
export const requiredUnless = (variable, expected, fieldName) =>
  helpers.withParams({ variable, expected }, function (value, data) {
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
...
export const sameAs = (field, fieldName) =>
  helpers.withParams({ field }, function (value, data) {
    const level = fieldName.split(".").length - 1;
    const dataWithParent = this.getDataAccordingToFieldLevel(
      this.getRootScreen().addReferenceToParents(data),
      level
