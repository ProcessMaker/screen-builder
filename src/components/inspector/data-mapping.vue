  ...
  <div>
    <div class="d-flex mb-1">
      <label class="flex-grow-1 m-0">{{
        $t("Output Variable Property Mapping")
      }}</label>
      <button
        type="button"
    ...
    <table class="table table-striped table-sm border mb-1">
      <thead>
        <tr>
          <th scope="col">
            <div class="float-left">{{ $t("Source") }}</div>
            <mustache-helper class="float-left ml-2" />
          </th>
          <th scope="col">
            <div class="float-left">{{ $t("Form Variable") }}</div>
            <mustache-helper class="float-left ml-2" />
          </th>
          <th scope="col">&nbsp;</th>
        </tr>
        ...
        <tr v-for="(row, i) in dataMapping" :key="i">
          ...
          <td class="p-1">
            <input
              ...
              name="key"
              :placeholder="
                $t('The Request variable is a new/existing variable')
              "
              type="text"
              class="form-control"
            />
    ...
    <small class="form-text text-muted mb-3">
      ...
      {{
        $t(
          "(If empty, all data returned will be mapped to the output variable)"
        )
      }}
    </small>
  </div>
...
export default {
  ...
  watch: {
    ...
    value: {
      immediate: true,
      handler() {
        const { dataMapping } = this.getConfig();
        if (dataMapping) {
    ...
    loadOptions() {
      ...
      window.ProcessMaker.apiClient
        ...
        .then((response) => {
          ...
          const mappings = window._.get(endpointData, "dataMapping", []);
