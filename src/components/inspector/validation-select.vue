        ...
        <b-card v-for="(rule, index) in rules" class="mb-2" :key="index">
          ...
          <b-card-header header-tag="header" class="p-0" role="tab">
            <div class="p-1 d-flex justify-content-between align-items-center">
              {{ rule.content }}
              <div class="actions">
                <b-button variant="link" class="p-0 mr-1 secondary" v-if="rule.configs" v-b-toggle="formatRuleContentAsId(rule.content)" data-cy="edit-rule">
                  <i class="fas fa-cog fa-fw text-secondary"/>
                </b-button>
                <b-button variant="link" class="p-0" @click="confirmDelete(index)" data-cy="remove-rule"><i class="fas fa-trash-alt fa-fw text-secondary"/></b-button>
              </div>
            </div>
          </b-card-header>
