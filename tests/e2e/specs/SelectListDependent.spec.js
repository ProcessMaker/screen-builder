describe("select list mustache", () => {
  beforeEach(() => {
    ...
    cy.intercept({
      ...
      onRequest: ({ xhr, request }) => {
        ...
        xhr.setRequestHeader("X-Cypress-Response", `"response":${JSON.stringify(response)}}`);
      },
      response: '{"status": 200'
    });
  ...
  it.skip("Verify Load values in multiselect list mustache + collection", () => {
    ...
    cy.get('[data-cy="screen-field-address"]').selectOption("My address in La Paz 1");
    cy.get('[data-cy="screen-field-country"]').selectOption("United States");
    // Once selected a different country city should be cleaned
    cy.get('[data-cy="screen-field-city"] .multiselect__placeholder').should("be.visible");
    cy.get('[data-cy="screen-field-address"] .multiselect__placeholder').should("be.visible");
    cy.get('[data-cy="screen-field-city"]').selectOption("Asheville");
    cy.get('[data-cy="screen-field-address"]').selectOption("My address in Asheville 2");
