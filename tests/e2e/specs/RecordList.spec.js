...
describe("Record list", () => {
  ...
  it("Invalid default values", () => {
    ...
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=fullname]").should(
      "have.value",
      "Miss Patricia Smith"
    );
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=date]")
      .parent()
  ...
  it("Recordlist pagination when deleting items should go to previous page if no records in current page", () => {
    ...
    cy.get("[data-cy=table-pagination]").should("be.not.exist");

    for (let i = 1; i <= 5; i++) {
      cy.get('[aria-rowindex="' + i + '"] > .table-column').should("contain.text", i);
    }
  });
  ...
  it("Check editing the correct record in recordlist after sorting", () => {
    ...
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [name=name]").should(
      "have.value",
      "A"
    );
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] button.btn-primary").click();
  ...
  it("FileUpload in record lists within loops", () => {
    ...
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=form_input_4]")
      .eq(0)
      .clear()
      .type("First");
    cy.intercept(
    ...
    cy.wait(1000);
    cy.get('[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [data-cy="screen-field-file_upload_2"]')
      .eq(0)
      .should("contain.text", "avatar.jpeg");
    cy.get('[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [data-cy="screen-field-file_upload_2"]')
      .eq(1)
    ...
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [name=form_input_4]")
      .eq(2)
      .clear()
      .type("Third");
    cy.intercept(
  ...
  it.skip("Check editing records with empty from second page", () => {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      if (data[i].date !== "") {
        cy.get(
          '[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [data-cy="screen-field-date"]input').click(
        ).type(data[i].date);
      }
      if (data[i].name !== "") {
        cy.get("[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=name]").type(data[i].name);
    ...
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [data-cy=screen-field-date] .form-control"
    )
