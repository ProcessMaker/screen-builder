...
describe("Complex screen", () => {
  ...
  const now = moment.utc().hours(8).minutes(15).seconds(0).milliseconds(0);
  ...
  function goToNextPage(selector) {
    cy.get(
      "[data-cy=preview-content] button:contains(Page Navigation >>)"
    ).click();
  ...
  beforeEach(() => {
    ...
    cy.intercept(
      "GET",
      "/api/1.0/requests/1/files?id=1",
      JSON.stringify({ data: files.page1 })
    );
    cy.intercept(
      "GET",
      "/api/1.0/requests/1/files?id=2",
      JSON.stringify({ data: files.page2 })
    );
  ...
  it("fills all pages", () => {
    ...
    cy.get("[data-cy=preview-content] [name=form_input_2]")
      .eq(0)
      .clear()
      .type("input in loop 1");
    cy.get("[data-cy=preview-content] [name=form_input_2]")
      .eq(1)
      .clear()
      .type("input in loop 2");
    cy.get("[data-cy=preview-content] [name=form_input_2]")
      .eq(2)
      .clear()
      .type("input in loop 3");

    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=add-row]"
    ).click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=first_name]"
    )
      .clear()
      .type("Thomas");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-add] [name=last_name]"
    )
      .clear()
      .type("Anderson");
    cy.get(
    ...
    cy.get(
      ...
      .click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [name=first_name]"
    ).type(" A.");
    cy.get(
    ...
    cy.uploadFile(
      '[data-cy=preview-content] [data-cy="screen-field-page1"] input[type=file]',
      "avatar.jpeg",
      "image/jpg"
    );
    // Mock uploaded file
    ...
    // Page 2
    // Next Page
    goToNextPage("[data-cy=preview-content] [name=form_input_5]");

    cy.get("[data-cy=preview-content] [name=form_input_5]").type(
      "form input 5"
    );

    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_select_list_4"]'
    ).selectOption("foo");

    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_date_picker_7"]'
    ).pickTodayWithTime("8", "15", "AM");

    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_3] [data-cy=add-row]"
    ).click();
    cy.get("[data-cy=modal-not-assigned] button.btn-primary").click();

    cy.get("[data-cy=preview-content] [name=form_text_area_4]").type(
      "form text area 4"
    );
    ...
    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_date_picker_8"]'
    ).pickTodayWithTime("8", "15", "AM");

    cy.get("[data-cy=preview-content] [name=form_input_6]")
      .eq(0)
      .clear()
      .type("input 6 in loop 1");
    cy.get("[data-cy=preview-content] [name=form_input_6]")
      .eq(1)
      .clear()
      .type("input 6 in loop 2");
    cy.get("[data-cy=preview-content] [name=form_input_6]")
      .eq(2)
      .clear()
      .type("input 6 in loop 3");

    cy.get("[data-cy=preview-content] [name=form_input_4]").type(
      "form input 4"
    );
    ...
    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_date_picker_6"]'
    ).pickTodayWithTime("8", "15", "AM");
    ...
    cy.uploadFile(
      '[data-cy=preview-content] [data-cy="screen-field-page2"] input[type=file]',
      "avatar.jpeg",
      "image/jpg"
    );
    // Mock uploaded file
    ...
    let loop;
    // Next Page
    goToNextPage("[data-cy=preview-content] [name=form_input_3]");

    // FILL LOOP 1
    loop = 0;
    cy.get("[data-cy=preview-content] [name=form_input_3]")
      .eq(loop)
      .type("info@processmaker.com");
    cy.get("[data-cy=preview-content] [name=form_text_area_2]")
      .eq(loop)
      .type("form text area 2");
    cy.get(
    ...
    cy.get("[data-cy=preview-content] [name=form_checkbox_2]").eq(loop).click();
    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_date_picker_3"]'
    )
      .eq(loop)
      .pickToday();
    cy.get(
      '[data-cy=preview-content] [data-cy="screen-field-form_date_picker_4"]'
    )
      .eq(loop)
      .pickTodayWithTime("8", "15", "AM");
    ...
    // fill grid
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=add-row]"
    )
      .eq(loop)
      .click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=modal-add] [name=first_name]"
    )
      .eq(loop)
      .clear()
      .type("Thomas");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=modal-add] [name=last_name]"
    )
      .eq(loop)
      .clear()
      .type("Anderson");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=modal-add] button.btn-primary"
    )
      .eq(loop)
      .click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=edit-row]"
    )
      .eq(loop)
      .click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=modal-edit] [name=first_name]"
    )
      .eq(loop)
      .type(" A.");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_2] [data-cy=modal-edit] button:contains(Save)"
    )
      .eq(loop)
      .click();

    cy.get("[data-cy=preview-content] [name=season]")
      .eq(loop)
      .clear()
      .type("1999");
    cy.get("[data-cy=preview-content] [name=season]")
    ...
    // Page 4
    // Next Page
    goToNextPage("[data-cy=preview-content] [name=form_input_1]");
    ...
    cy.get("[data-cy=preview-content] [name=form_input_6]")
      .clear()
      .type(moment(now).format("YYYY-MM-DD HH:mm"));
    ...
    cy.get("[data-cy=preview-content] [name=form_checkbox_1]").click();
    cy.get("[data-cy=preview-content] [name=form_checkbox_2]").should(
      "be.checked"
    );
    ...
    cy.get('[data-cy=preview-content] [id^="form_select_list_3-b-"]').click();
    cy.get('[data-cy=preview-content] [id^="form_select_list_4-b-"]').click(); // Select b
    cy.get('[data-cy=preview-content] [id^="form_select_list_4-c-"]').click(); // Select c
    // record list - complete new fields
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=edit-row]"
    ).click();
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [name=email]"
    )
      .clear()
      .type("thomas@processmaker.com");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] [name=url]"
    )
      .clear()
      .type("https://thomas.processmaker.com");
    cy.get(
      "[data-cy=preview-content] [data-cy=screen-field-form_record_list_1] [data-cy=modal-edit] button:contains(Save)"
    ).click();
    // checkboxes
    ...
    // inputs in a loop
    cy.get("[data-cy=preview-content] [name=form_input_9]")
      .eq(0)
      .clear()
      .type("input in loop 1");
    cy.get("[data-cy=preview-content] [name=form_input_9]")
    ...
    cy.uploadFile(
      '[data-cy=preview-content] [data-cy="screen-field-nested_file"] input[type=file]',
      "avatar.jpeg",
      "image/jpg"
    );
    // Mock uploaded file
