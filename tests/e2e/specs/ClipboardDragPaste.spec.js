import { addControlInsideLoop, addControlInsideTable } from "../support/utils";
import { nodeControls } from "../support/constants";

describe("Clipboard Drag and Paste", () => {
  function visitWithCleanStorage() {
    cy.visit("/", {
      timeout: 120000,
      onBeforeLoad(win) {
  ...
  it("Paste multicolumn clipboard item to a different screen", () => {
    ...
    cy.get('[data-cy="screen-element-container"]').should("have.length", 1);
    cy.get('[data-cy="screen-element-container"] .column-draggable').should(
      "have.length",
      2
    );
    cy.get('[data-cy="screen-element-container"] .column-draggable')
      .eq(0)
      .find("textarea")
      .should("exist");
  ...
  it("Copy element from inside multicolumn to clipboard", () => {
    cy.window().then((win) => {
      ...
      cy.get(
        '[data-cy="screen-element-container"] [class="control-item"]'
      ).click();
