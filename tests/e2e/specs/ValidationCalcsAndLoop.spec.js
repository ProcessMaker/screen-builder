describe("Validation Calcs properties and loop", () => {
  ...
  it("Data is fully accessible from a collection screen", () => {
    ...
    cy.get('[data-cy=preview-content] [data-cy="screen-field-foo.two"]').parent().should("contain.text", "two");

    cy.get(":nth-child(2) > .form-group > :nth-child(1) > div > p").should("contain.text", "output: one");
  });
