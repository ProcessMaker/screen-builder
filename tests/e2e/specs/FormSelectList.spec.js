describe("Form Select List", () => {
  ...
  it("Set data source values", () => {
    ...
    cy.get("[data-cy=preview-content] span:contains(three)").should(
      "not.exist"
    );
  ...
  it("Validate duplicated option", () => {
    ...
    cy.get("[data-cy=inspector-option-content]").type("one");
    cy.get("[data-cy=inspector-option-save]").click();
    cy.get('div:contains("An item with the same key already exists")').should(
      "be.visible"
    );
  ...
  it("Options from Data Connector (single value)", () => {
    ...
    cy.get("[data-cy=preview-content] span:contains(John)").should(
      "be.visible"
    );
    cy.get("[data-cy=preview-content] span:contains(Mary)").should(
      "be.visible"
    );
    cy.get("[data-cy=preview-content] span:contains(Patricia)").should(
      "be.visible"
    );
