describe("Watchers", () => {
  ...
  it("Create duplicated properties", () => {
    ...
    cy.get('[data-cy="watchers-watcher-variable"] input')
      .clear()
      .type("form_input_1");
    cy.get('[data-cy="watchers-watcher-variable"]').should(
      "contain.text",
      "No elements found."
    );
    cy.get('[data-cy="watchers-button-save"]').click();
    cy.get("#watcherConfig").should(
      "contain.text",
  ...
  it("Test error in synchronous watcher", () => {
    ...
    cy.get("#watchers-synchronous").should(
      "contain.text",
  ...
  it.skip("Test watcher with select list", () => {
    ...
    // Select option
    cy.get('[data-cy="screen-field-form_select_list_1"]').selectOption("Musk");
  ...
  it("Focuses the first field that has an error", () => {
    ...
    cy.get(
      '[data-cy="watchers-watcher-source"] .multiselect__content-wrapper'
    ).should("be.visible");

    cy.get("#option-1-3").click(); // Select "script" source
    cy.get('[data-cy="watchers-watcher-input_data"] .monaco-editor')
      .click()
      .focused()
      .type("abc");
    cy.get('[data-cy="watchers-accordion-output"]').click({
