...
describe("Validation Rules", () => {
  ...
  it("Invalid default values", () => {
    ...
    // same
    cy.get("[data-cy=preview-content] [data-cy=screen-field-form_date_picker_1]").should("contain.text", "Must be equal or before today");
    cy.get('[data-cy=preview-content] [data-cy="screen-field-form_date_picker_1"]').pickToday();
    cy.get('[data-cy=preview-content] [data-cy="screen-field-form_date_picker_1"]').should(
      "not.contain.text",
      "Must be equal or before today"
    );
  ...
  it("Validations should be skipped for readonly fields", () => {
    ...
    cy.get('[data-cy=editor-content] [name="form_input"]').parent().find(".invalid-feedback").should("be.visible");

    cy.get('[data-cy=editor-content] [name="form_input_readonly"]').parent().find(".invalid-feedback").should("be.not.exist");

    // In preview: ensure standard required field displays error while readonly required field does not
    cy.get("[data-cy=mode-preview]").click();

    cy.get('[data-cy=preview-content] [name="form_input"]').parent().find(".invalid-feedback").should("be.visible");

    cy.get('[data-cy=preview-content] [name="form_input_readonly"]').parent().find(".invalid-feedback").should("be.not.exist");
    ...
    cy.get('[data-cy=preview-content] [name="form_input"]').type("text");
