describe("RAOS 1.0.0 Account Opening Customer Form", () => {
  ...
  it("Verify Calcs with visibility rules update immediately on the same screen", () => {
    ...
    // add a new account
    cy.get("[data-cy=preview-content] [data-cy='loop-additionalAccounts-add']").click();
    // select the first account type: "Checking Account"
    cy.get("[data-cy=preview-content] [data-cy='screen-field-productType']").selectOption("Checking Account");
    // click on continue
    cy.get("[data-cy=preview-content] [aria-label='Continue']").eq(0).click();
    // choose funding method
    cy.get("[data-cy=preview-content] [data-cy='screen-field-fundingMethod']").eq(0).click();
    // "initialDepositOther" field must be visible
    cy.get("[data-cy=preview-content] [name='initialDepositOther']").eq(0).should("be.visible");
