...
function fillInputText(dataCy, index = null, value = "test") {
  if (index === null) {
    cy.get(`[data-cy=preview-content] [data-cy="${dataCy}"]`).clear().type(value);
  } else {
    cy.get(`[data-cy=preview-content] [data-cy="${dataCy}"]`).eq(index).clear().type(value);
  }
}

function shouldHaveValidationErrors() {
  cy.get("#preview .form-group.form-group--error:visible").should("have.length.greaterThan", 0);
}

function shouldNotHaveValidationErrors() {
  cy.get("#preview .form-group.form-group--error:visible").should("have.length", 0);
}
