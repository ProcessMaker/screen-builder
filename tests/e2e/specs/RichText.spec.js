describe('Rich Text control', () => {
  it('Rich text with mustache', () => {
    ...
    cy.get('[data-cy=inspector-content]').clear({force: true}).type('<p>Hello {{ name }}</p>', {parseSpecialCharSequences: false});
  ...
  it('Rich text render HTML from a Variable', () => {
    ...
    cy.get('[data-cy=controls-FormHtmlViewer]').drag('[data-cy=screen-drop-zone]', 'bottom');
    cy.get('[data-cy=screen-element-container]').click();
    cy.get('[data-cy=inspector-content]').clear({force: true}).type('{{ name }}', {parseSpecialCharSequences: false});
