describe('Responsive Preview test', () => {
  ...
  beforeEach(() => {
    ...
    cy.get(controlInputSelector).drag(screenDropZoneSelector, 'bottom');
    cy.get('[data-cy=controls-FormSelectList]').drag(screenContainerSelector, 'bottom');
  });

  it('should render the mode editor and mode preview buttons', () => {
    // Mode buttons
    cy.get(modeEditorSelector).should('be.visible').and('have.class', 'btn-secondary');
    cy.get(modePreviewSelector).should('be.visible').and('not.have.class', 'btn-secondary');
    // Device buttons
  ...
  it('should render the device screen buttons when clicking on mode preview', () => {
    ...
    // Device buttons
    cy.get(desktopButtonSelector).should('be.visible').and('have.class', 'btn-secondary');
    cy.get(mobileButtonSelector).should('be.visible').and('not.have.class', 'btn-secondary');
  });
  ...
  it('should render the screen renderer as a desktop device when clicking on the desktop button after clicking on the mobile button', () => {
    ...
    // Screen container
    cy.get(screenRendererSelector).should('be.visible').and('have.class', 'container-desktop');
  });
  ...
  it('should render the screen renderer as a mobile device when clicking on the mobile button after clicking on the mode editor', () => {
    ...
    cy.get(modePreviewSelector).click();
    // Desktop button
    cy.get(desktopButtonSelector).should('be.visible').and('not.have.class', 'btn-secondary');
    // Mobile button
    cy.get(mobileButtonSelector).should('be.visible').and('have.class', 'btn-secondary');
