describe("Form Image", () => {
  ...
  it('Image by variable', () => {
    ...
    cy.setPreviewDataInput({image: 'https://media.istockphoto.com/id/1398473177/es/foto/cuestionario-con-casillas-de-verificaci%C3%B3n-completar-el-formulario-de-la-encuesta-en-l%C3%ADnea.jpg?s=1024x1024&w=is&k=20&c=VU8-OYtDgwGzM1C4Vr-NGsnKHDEZGxzGCNe-WlzUFx8='});
  ...
  it("Image 64 bits to blob", () => {
    ...
    // Add image to loop
    cy.get("[data-cy=controls-FormImage]").drag(
    ...
    cy.get("[data-cy=mode-preview]").click();
    cy.get("[data-cy=preview-content] img").should("have.attr", "src");
    // cy.get('[data-cy=preview-content] img').should('have.length', 3);

    cy.get("[data-cy=preview-content] img")
      .eq(0)
      .invoke("attr", "src")
      .should("contain", "blob");
    cy.get("[data-cy=preview-content] img")
      .each(($el, index, $list) => {
        // console.log($el);
        cy.wrap($el).invoke("attr", "src").should("contain", "blob");
      })
      .then(($lis) => {
