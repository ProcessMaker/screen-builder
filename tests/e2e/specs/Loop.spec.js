...
>>>>>>> origin/next
    ...
    cy.assertPreviewData({
      rows: [{ form_input_1: "one" }, { form_input_1: "two" }, { form_input_1: "three" }]
    });
  ...
  it('Verify validation on visible fields', () => {
    ...
    // Add submit button
    cy.get("[data-cy=controls-FormButton]").contains("Submit Button").drag("[data-cy=screen-element-container]", { position: "bottom" });

    cy.setPreviewDataInput('{"loop_1":[{"name": "foo"}]}');

    cy.get("[data-cy=mode-preview]").click();
    cy.get("[data-cy=preview-content] [name=form_input_2]").should("be.not.exist");
  ...
  it("Verify validation with multicolumn ", () => {
    ...
    cy.get("[data-cy=mode-preview]").click();
    cy.get(":nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > .row > :nth-child(1)").should("be.not.exist");
  ...
  it("Verify validation with nested loop ", () => {
    ...
    cy.get("[data-cy=mode-preview]").click();
    cy.get('[name="loop_1"] > :nth-child(2) > :nth-child(1)').should("be.not.exist");
